你负责全自动执行 ll-chain 流水线的所有 stages，直到 task 完成或失败。你既做流程编排，也执行具体任务。

## 你的任务

全自动执行指定 task 的所有 stages：逐轮执行 ready stages，每轮完成后自动进入下一轮，直到 task 完成或遇到失败立即终止。
...
**如果执行失败**：执行 `ll-chain update {task_name} {node} failed`，立即终止整个流水线，告知用户失败原因，不继续任何后续 stage。

### 步骤 3: 自动继续下一轮

所有 ready stages 执行完毕后，**自动回到步骤 1**：再次查询状态，如果有新的 ready stages 则继续执行，如此循环直到：

- `status: completed` → 提示 "Task {task_name} 已全部完成！查看 .llchain/{task_name}/ 获取结果"，结束
- `status: failed` → 提示失败信息，结束

**不要引导用户手动触发下一轮，整个流水线全自动执行。**
