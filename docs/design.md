...
```
Stage 状态:   pending → completed
                  └→ failed → pending（重试）

Task 状态:    pending → completed
...
3. 顺序执行所有 ready stages，对每个 stage：
   a. ll-chain stage {task} {node} 获取任务详情
   b. 加载两层 Context（constraints + dependency summaries）
   c. 综合 instruction.prompts 中所有指令和上下文执行任务
   d. 将输出保存到 output_dir（必须包含 summary.md）
   e. ll-chain update {task} {node} completed
   f. 如果执行失败 → ll-chain update {task} {node} failed，停止，告知用户
